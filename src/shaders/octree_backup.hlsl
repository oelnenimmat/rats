...
VoxelData get_octree_voxel(ivec3 voxel, int in_depth, out int out_depth)
{
...
VoxelData traverse_octree(const Ray ray)
{
	...
	// We don't use this, but it helps to word out where voxel_to_world and world_to_voxel come from
	vec3 voxels_inside_world_unit = vec3(voxels_in_world) / world_max;

	// float scale = get_uniform_scale();
	vec3 voxel_to_world = vec3(1,1,1) / voxels_inside_world_unit;  //voxel_info.voxel_to_world.xyz;
	vec3 world_to_voxel = voxels_inside_world_unit;  //voxel_info.world_to_voxel.xyz;
	...
	const vec3 start_VS = start_WS * world_to_voxel;
	const vec3 direction_VS = normalize(ray.direction * world_to_voxel);
	...
	const ivec3 just_out = ivec3(
		dir.x < 0 ? -1 : voxels_in_world.x,//voxel_info.voxels_in_chunk.x * voxel_info.chunks_in_world.x,
		dir.y < 0 ? -1 : voxels_in_world.y,//voxel_info.voxels_in_chunk.y * voxel_info.chunks_in_world.y,
		dir.z < 0 ? -1 : voxels_in_world.z //voxel_info.voxels_in_chunk.z * voxel_info.chunks_in_world.z
	);
	...
	while(sanity_check > 0)
	{
		...
		int depth;
		VoxelData data = get_octree_voxel(voxel, max_sample_depth, depth);
		// VoxelData data = get_voxel(voxel);
		if (get_material(data) > 0)
		{
			data.color.xyz *= float(voxel.x) / (voxels_in_world.x - 1);
			return data;
		}
	
		// int power = max_sample_depth - depth;
		// int transform = 1 << power;
...
VoxelData traverse_voxels(const Ray ray)
{
	...
	vec3 voxel_to_world = voxel_info.voxel_to_world.xyz;
	vec3 world_to_voxel = voxel_info.world_to_voxel.xyz;
	...
	// VS = Voxel Space, where there is one voxel per unit
	const vec3 start_VS = start_WS * world_to_voxel;
	vec3 direction_VS = normalize(ray.direction * world_to_voxel);
