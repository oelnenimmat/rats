...
vec4 _traverse_chunktree(const Ray ray, float max_distance, int map_index, out float out_depth)
{
	...
	while(sanity_check > 0)
	{
		...
		if (chunk_is_empty(voxel, map_index))
		{
			...
			// into consideration
			ivec3 voxel_offset_from_full_chunks = voxel_objects[map_index].offset_in_voxels.xyz % get_voxels_in_chunk();
		...
		if (material > 0)
		{
			...
			else if (draw_mode == DRAW_MODE_TOON)
			{
				...
				vec3 direct_light = step(0, dot(-per_frame.lighting.direct_direction.xyz, data.normal.xyz)) * per_frame.lighting.direct_color.rgb;
